...
function checkRow(word){
    ...
    let interval = setInterval(function(){
        ...
        checkChar(char, index, row, tmpWord);
        if(index + 1 === 5){
...
function checkChar(currentChar, index, row, tmpWord){
    ...
    else if (currentChar != tmpWord["word"][index].toUpperCase() && tmpWord["word"].includes(currentChar)){
    ...
    if (index == 4){
        if(userWord.toString() === tmpWord["word"].toString() && currentRow <= 6){
            $(".status").text("You Win!")
            setTimeout(function(){
                location.reload();
            },2000);
...
function wordData(currentWord){
    ...
    for (let i = 0; i < currentWord.length; i++){
        if (wordValues[currentWord[i]] == undefined){
            wordValues[currentWord[i]] = 1;
        }
        else{
            wordValues[currentWord[i]] += 1;
        }
